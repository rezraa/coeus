...
2. **RECOGNIZE, then CALL.** Both `analyze_architecture` and `recommend_pattern` drive off the signal index — the one proven problem-language → pattern path.
   - **CALL** `get_signal_index` — it returns the uniform nested container `{pattern_signals: [{signal_id, signal_text, pattern_ids}, ...]}` (one named group per corpus; Coeus has one merged pattern corpus today, so read `pattern_signals`). Recognize which of the problem's structural signals match the `signal_text`s in that group — collect their `signal_id`s.
